...
The goal of EBArulebook is to scrape the [single rulebook](https://eba.europa.eu/regulation-and-policy/single-rulebook) and [Q&As](https://eba.europa.eu/single-rule-book-qa) published by the European Banking Authority (EBA).

## Installation

You can install the released version of EBArulebook from [CRAN](https://CRAN.R-project.org) with:
...
Code in this repository was used to acquire and analyse data used in the forthcoming Staff Working Paper. Check the vignette for more details.
...
This scraper was tested only on the Capital Requirements Regulation (CRR) but it should also work on other parts of the rulebook.
...
The rulebook is also using simple html to display the rules and is constructed in a simple way:
...
2. Extract the relevant numbers by running regex on the scraped html file. Look for the IDs: "article-id/[DIGIT]"
...
Search for questions. There are 1757 Q&As (1652 Final and 105 Rejected) as of 8 June 2019.
Max displayed per page is 200 (end of the URL) so 9 pages in total (see: 'cur=2' in the URL)
...
Status: use 'All': both 'Final' and 'Rejected'
...
Extract URL to the actual questions from the downloaded website using `parse_EBA_QA_page.R`
